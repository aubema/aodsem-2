C
C   LOGICIEL bmies.f QUI PERMET DE TRAITER UNE SERIE DE CAS A L AIDE
C   de la routine DE CALCUL DE MIE mies.f.  LE CALCUL EST  
C   EFFECTUE SUR UN SEUL BIN A LA FOIS.  
C   LE LOGICIEL A ETE CONCU POUR PRODUIRE UNE BASE DE DONNEE 
C   CARACTERISANT LES SECTIONS EFFICACES MOYENNE DE CHAQUE BIN NARCM 
C   POUR DIFFERENTES LONGUEURS D ONDES ET DIFFERENTS TYPES D AEROSOLS.
C   LES PARAMETRES CHANGEANTS SONT
C   LES INDICES DE 
C   REFRACTION (FICHIER NOM.mie.ref) ET LE FICHIER
C   DE BINS (NOM.mie.bns).
C
C   FORMAT DES FICHIERS D ENTREE
C
C   Martin Aube 28-01-2000
C
C   NOM.ref.bmi
C
c  4                     * NUMBER OF AEROSOL TYPES*
c  7 			 * NUMBER OF WAVELENGTH*
c  SEASALT               * AEROSOL TYPE *
c  440 .44 1.5 2.544e-08 * WAVEL.ID,WAVEL.(um),R.REF.IND.,IMM.REF. IND.*
c  500 .50 1.5 1.55e-08
c  550 .55 1.5 1e-08
c  670 .67 1.49 1.0544e-07
c  860 .86 1.48 2.622e-05
c  940 .94 1.476 8.184e-05
c  1020 1.02 1.47 3.146e-04
c  SULFATE               * AEROSOL TYPE, R.H. *
c  440 .44 1.53 5e-03    * WAVEL.ID,WAVEL.(um),R.REF.IND.,IMM.REF. IND.*
c  500 .50 1.53 5e-03
c  550 .55 1.53 6e-03
c       .
c       .
c       .
c  water
c  440 0.44	1.3373E+0	1.4702E-9
c  500 0.5	1.3345E+0	1.0765E-9
c  550 0.55	1.3330E+0	2.1141E-9
c  670 0.67	1.331422	2.3788E-8
c  860 0.86	1.329036	3.2360E-7
c  940 0.94	1.32783		1.8309E-6
c  1020 1.02	1.326615	3.3905E-6
c
c
c   NOM.bns.bmi
c
c  10           * NUMBER OF SUB-SIZE-BIN DIVISIONS FOR CALCULATIONS *
c  0            *SUB-BIN PSD VALUES(0=0 moment,2=LOG-NORM.,3=1st ...
c  12           *NOMBRE DE BINS*
c  1 0.005 0.01 *BIN NO, RMIN, RMAX*
c  2 0.01 0.02
c  3 0.02 0.04
c  4 0.04 0.08
c  5 0.08 0.16
...
c  12 10.24 20.48
C
C   Le programme considere que chaque bin est Nexp fois plus grand que le 
c   precedent.  Si tel n'est pas le cas, une legere erreur sera rencon-
c   tree lors du calcul de la croissance hygrometrique des bornes de
c   chaque bin car cet algorithme est optimise pour des 
c   tailles=largeur1*Nexp^(NB-1)
c   Deplus il faut s'assurer que les bins de hum.f correspondent exactement
c   aux bins du present run
c
c
      PROGRAM batchmie
C
...
C
C   nr=10,nrh=13,nb=12,nw=14
C
      CHARACTER*60 NOMREF,NOMBNS,BPSDFILE,MIEFILE,nomres,BIDON,mietemp
      character*60 psdtemp, nompfct, humname(10)
      CHARACTER*8 NOM
      CHARACTER*2 rh(13)
      CHARACTER*60 BINFLAG(120), WAVEFLAG(14), NOMTYPE(10)
      INTEGER LENNOM, NWAV, NREF, NBNS, LENTYPE(10), BIMPRIMER,BITYPE
      INTEGER BPOL,BIPSD,BNBINS,Binterp,LENPSDF,LENWFLAG(14),BNANG
      INTEGER LENBFLAG(120),BINTERAC,NB,NR,NW,NA,nrh,nh,nhum,nbnhum
      INTEGER nt,ntype,nty,intetype,percent,pcentold,nbrh
      REAL*8 WAVELEN(14),INDREFR(10,14),INDREFI(10,14), RAYON1(120)
      REAL*8 RAYON2(120),BANGLEL,BANGLEU,BDELTA,BREM,BIMM,BRINC,BWAVLEN
      REAL*8 BPSD,BNBINTER,X1(120),X2(120),THETA(360),mr(10,13,14)
      REAL*8 mi(10,13,14),rratio(10,13,120),miwater(14),mrwater(14)
      REAL*8 PHAFCT(180,14,120,10,13),SIGMAE(14,120,10)
      REAL*8 SIGMAA(14,120,10)
      REAL*8 SIGMAS(14,120,10),RMO(10),SIG(10),RM,SG,x1rh,x2rh
      REAL*8 radrat(10,13,120),finden(10,13,120)
C
...
C
     ...
     +'98','99'/
      ...
      BINTERAC=0
C
C   CHOIX DU NOM DE BATCH
C
      PRINT*,'ROOT NAME OF THE BATCH FILE ?'  
      READ(*,*) NOM
C
...
C
      NOMREF=NOM(1:LENNOM)//'.ref.bmi'
      NOMBNS=NOM(1:LENNOM)//'.bns.bmi'
C
C   LECTURE DES INDICES DE REFRACTION 
C
      OPEN(UNIT=12,FILE=NOMREF,STATUS='OLD')
         print*,'Reading refractive index file: ',NOMREF
         READ(12,*) NREF
         READ(12,*) NWAV
         PRINT*,'Number of aerosols types: ',NREF
         PRINT*,'Number of wavelength: ',NWAV
         DO 100 NR=1,NREF
            read(12,*) NOMTYPE(NR)
            LENTYPE(NR)=INDEX(NOMTYPE(NR),' ')-1
            do 200 NW=1,NWAV
               READ(12,*) WAVEFLAG(NW),WAVELEN(NW),INDREFR(NR,NW),
     +         INDREFI(NR,NW)
               LENWFLAG(NW)=INDEX(WAVEFLAG(NW),' ')-1
 200        CONTINUE
 100     CONTINUE
c
c
c   lecture de l indice de refraction de l eau
c
c
               read(12,*) bidon
            do 203 NW=1,NWAV
               READ(12,*) bidon,bidon,mrwater(NW),
     +         miwater(NW)
 203        CONTINUE
      ...
      OPEN(UNIT=18,FILE='humout',STATUS='OLD')
         ...
         do 104 nt=1,ntype
            ...
            do 204 nh=1,nhum
               ...
               read(18,*) bidon
               do 304 nb=1,nbnhum
                  read(18,*) bidon,radrat(nt,nh,nb),finden(nt,nh,nb)
 304           continue
 204        continue
 104     continue
      CLOSE(UNIT=18)
...
C   LECTURE DES BINS
C
       OPEN(UNIT=13,FILE=NOMBNS,STATUS='OLD')
         print*,'Reading size-bin file: ',NOMBNS
C
C   EN COMBIEN DE SOUS-BINS VOULEZ-VOUS DIVISER LES PSDs 
C   (MIN:1,MAX:5000.)
C   dans le cas de bmies cela revient a dire en combien d intervalle
C   de calcul nous voulons diviser le bin.
C   Nous recommandons une valeur de 20 dans un souci de temps de
C   calcul raisonnable versus precision raisonnable.
C
         read(13,*) BNBINTER
         print*,'Number of size-bin divisions: ',BNBINTER
c
c  La valeur de Binterp est passee a la routine mies qui l utilise pour
c  definir la distribution de la psd a l interieur du bin.
c 
c  0=valeur constante sur tout le bin (moment 0)
c  1=distribution lineaire entre la valeur du bin et la valeur du bin
c    suivant.  Cette option n est pas utilisee dans bmies car nous ne 
c    considerons qu un seul bin a la fois.
c  2=la valeur de la psd suit une distribution log-normale caracterisant
c    le type de particule a partir du WCP-55 ou de Shettle et Fenn.
c  3=proprotionnel a 1/X (moment 0)
c  4=proportionnel a abs(log X)/X (moment 1)
c  9=valeur non nul uniquement pour la premiere partie du sous bin.
c    (seulement pour X=debut du bin)
c
         read(13,*) Binterp 
         print*,'Sub-bin PSD interpolation type: ',Binterp
         READ(13,*) NBNS
         PRINT*,'Number size-bins: ',NBNS
         DO 300 NB=1,NBNS
            READ(13,*) BINFLAG(NB),RAYON1(NB),RAYON2(NB)
            LENBFLAG(NB)=INDEX(BINFLAG(NB),' ')-1
 300     CONTINUE
      CLOSE(UNIT=13)
c
c   determination de l'interpolation log-normal pour chaque type d
c   aerosol.
      if (Binterp.eq.2) then
         do 301 NR=1,NREF
            print*,'Which kind of particles below is closer to yours'
            print*,'for ',NOMTYPE(NR)(1:LENTYPE(NR)),' ?'
            print*,'(The program will use WCP-55 or Shettle & Fenn 1979'
            print*,' log-normal size distribution to interpolate the' 
            print*,' sub-bin distributions.)'
            print*,' '
            print*,' 0 = Water soluble                      [WCP-55]'
            print*,' 1 = Dust-like                          [WCP-55]'
            print*,' 2 = Soot                               [WCP-55]'
            print*,' 3 = Sea salt (70 to 80%  R.H.) [Shettle & Fenn]'
            print*,' 4 = Volcanic ash                       [WCP-55]'
            print*,' 5 = Desert dust storm                  [WCP-55]'     
 1071       read*,intetype
            if (intetype.eq.0) then
               RMO(NR)=0.0285
               SIG(NR)=2.239
            elseif (intetype.eq.1) then
               RMO(NR)=0.471
               SIG(NR)=2.512
            elseif (intetype.eq.2) then
               RMO(NR)=0.0118
               SIG(NR)=2.0
            elseif (intetype.eq.3) then
               RMO(NR)=0.3
               SIG(NR)=2.51
            elseif (intetype.eq.4) then
               RMO(NR)=0.217
               SIG(NR)=1.77
            elseif (intetype.eq.5) then
               RMO(NR)=10.8
               SIG(NR)=2.74
            else 
               goto 1071
...
C   DEBUT DU CALCUL
C 
      print*,'Mie s calculations...'
      do 399 nrh=1,13
      DO 400 NB=1,NBNS
         print*,'Processing bin #',NB
         nty=0
         DO 500 NR=1,NREF
c
c   Association du facteur hygrometrique au bon type de particule (NR)
c
            nty=0
            do 503 nt=1,ntype
               if (humname(nt)(1:6).eq.NOMTYPE(NR)(1:6)) then
                  nty=nt
               endif
 503        continue
            if (nty.eq.0) then
               print*,'Incorrect aerosol type name <<',NOMTYPE(NR)(1:
     +         LENTYPE(NR)),'>> in .ref.bmi file'
               print*,'Allowed names: SEASALT, SULFATE, NITRATE,'
               print*,'               BLACKCARBON, SOILDUST'
               print*,'nty,nref',nty,nref
               stop
            endif
            SG=SIG(NR)
            RM=RMO(NR)
...
c
c
c    Facteur hygrometrique sur l indice de refraction
c
c    calcul moyen pour le centre du bin a l aide du fichier humout
c    
c

c   ponderation en resolvant le probleme a deux couches (a confirmer)
c               mr(NR,nrh,NW)=mrwater(NW)*INDREFR(NR,NW)/(INDREFR(NR,NW)-
c     + (mrwater(NW)-INDREFR(NR,NW))*(radrat(nty,1,nb)/radrat(nty,nrh,nb)
c     + )**3.)
c               mi(NR,nrh,NW)=miwater(NW)*INDREFI(NR,NW)/(INDREFI(NR,NW)-
c     + (miwater(NW)-INDREFI(NR,NW))*(radrat(nty,1,nb)/radrat(nty,nrh,nb)
c     + )**3.)

c  ponderation en proportion du volume
               mr(NR,nrh,NW)=mrwater(NW)+((INDREFR(NR,NW)-mrwater(NW))*
     +         (radrat(nty,1,nb)/radrat(nty,nrh,nb))**3.)
               mi(NR,nrh,NW)=miwater(NW)+((INDREFI(NR,NW)-miwater(NW))*
     +         (radrat(nty,1,nb)/radrat(nty,nrh,nb))**3.)

                BIMPRIMER=0
                BPSDFILE=NOM(1:LENNOM)//NOMTYPE(NR)(1:LENTYPE(NR))//
     +          'RH'//rh(nrh)//'Bin'//BINFLAG(NB)(1:LENBFLAG(NB))//'Lam'
     +          //WAVEFLAG(NW)(1:LENWFLAG(NW))
                LENPSDF=INDEX(BPSDFILE,' ')-1
                BITYPE=1
                BANGLEL=0
                BANGLEU=180
                BDELTA=1
                BNANG=1+(BANGLEU-BANGLEL)/BDELTA
                BPOL=3
                BIPSD=7
                BRINC=99999
                BNBINS=1
                ...
                BPSD=1.
c
c   Corriger les bornes du bin pour la croissance hygrometrique
c   et convertir les rayons en parametres de taille
c
c   Cette correction procede a l interpolation des facteurs de 
c   croissance hygrometrique calcules pour le centre de chaque bin 
c   dans le fichier humout.  Pour accelerer le traitement, 
c   l interpolation est adaptee au cas particulier ou la largeur 
c   du bin NB est Nexp fois plus grand que le bin NB-1 ce qui est le cas 
c   pour la definition des bins NARCM il faut donc s'assurer que la definition
c   des bins suivent cette dependance
c
        Nexp=RAYON1(NB+1)/RAYON1(NB)
        pond1=Nexp/(1.+Nexp)
        pond2=1./(1.+Nexp)
        if ((NB.gt.1).and.(NB.lt.NBNS)) then
           x1rh=RAYON1(NB)*(pond1*radrat(nty,nrh,nb-1)+pond2*
     +     radrat(nty,nrh,nb))
           x2rh=RAYON2(NB)*(pond1*radrat(nty,nrh,nb)+pond2*
     +     radrat(nty,nrh,nb+1))
           x1rh=2.*x1rh*PI/BWAVLEN
           x2rh=2.*x2rh*PI/BWAVLEN
        elseif (NB.eq.1) then
           x1rh=RAYON1(NB)*radrat(nty,nrh,nb)
           x2rh=RAYON2(NB)*(pond1*radrat(nty,nrh,nb)+pond2*
     +     radrat(nty,nrh,nb+1))
           x1rh=2.*x1rh*PI/BWAVLEN
           x2rh=2.*x2rh*PI/BWAVLEN
        else
           x2rh=RAYON2(NB)*radrat(nty,nrh,nb)
           x1rh=RAYON1(NB)*(pond1*radrat(nty,nrh,nb-1)+pond2*
     +     radrat(nty,nrh,nb))
           x1rh=2.*x1rh*PI/BWAVLEN
           x2rh=2.*x2rh*PI/BWAVLEN
...
c
C
C   CREATION DU FICHIER PSD
C
                OPEN(UNIT=14,FILE=BPSDFILE,STATUS='UNKNOWN')
       WRITE(14,*) BIPSD,' ** type de distribution de taille **'
       WRITE(14,*) BRINC,' ** Increment dX constant (99999=variable) **'
       WRITE(14,*) BNBINS,' ** nombre de bins de la distribution **'
       WRITE(14,*) Binterp,' ** Interpolation (0=terrasse, 1=linÝaire, 2
     +=taille 9=discret **'
       WRITE(14,*) BWAVLEN,' ** longueur d onde en microns **'
       WRITE(14,*) x1rh, BPSD,' ** X, Psd **'
       WRITE(14,*) x2rh,' ** X final **'
                CLOSE(UNIT=14)
C
C   ECRITURE DU FICHIER .mie
C
                OPEN(UNIT=15,FILE=BPSDFILE(1:LENPSDF)//'.mie',STATUS=
     +'UNKNOWN')  
 2200  FORMAT(I2,1X,' ** Morphologie de l aerosol **')
 ...
 2230  FORMAT(I2,1X,' ** etat de polarisation (1=//,2=_|_,3=random) **')
       ...
       WRITE(15,*) BIMPRIMER,' ** 1 = AFFICHAGE **'
       WRITE(15,*) BPSDFILE
       WRITE(15,2200) BITYPE
       WRITE(15,2220) BANGLEL,BANGLEU,BDELTA
       WRITE(15,2230) BPOL
       WRITE(15,2210) mr(NR,nrh,NW), mi(NR,nrh,NW)
                CLOSE(UNIT=15)
                 CALL MIES(RM,SG,BPSDFILE,BNBINTER,BINTERAC)
 600        CONTINUE
            percent=((nrh-1)*NBNS+NB)*100/(NBNS*13)
          if (((percent/5)*5.eq.percent).and.(percent.ne.pcentold)) then
               print*,percent,'%'
            endif
            pcentold=percent
 500     CONTINUE
...
c ---------------
c
c   Creation d un fichier unique .res
c
      nomres=NOM(1:LENNOM)//'.res.bmi'
      print*,'Writing cross section output file: ',nomres
      open(unit=16,file=nomres,status='unknown')
      write(16,603) NREF,nbrh,NWAV,NBNS
 603   format(I9,1x,I9,1x,I9,1x,I9,T50,
     +'#types,#R.H.,#wlength,#bins')
      DO 811 NR=1,NREF
         WRITE(16,601)  NOMTYPE(NR)(1:LENTYPE(NR))
 601     format(A19,1x,A9,T50,
     +   'Aerosol type')
         do 1009 nrh=1,13
            write(16,*) rh(nrh),'   Relative Humidity (%)'
         DO 1011 NW=1,NWAV
            WRITE(16,604) WAVELEN(NW),INDREFR(NR,NW),INDREFI(NR,NW)
 604        format(F9.5,1x,E12.5E2,1x,E12.5E2,T50,
     +      ' Wavelength (um),  mr,  mi')
            DO 911 NB=1,NBNS
               MIEFILE=NOM(1:LENNOM)//NOMTYPE(NR)(1:LENTYPE(NR))//
     +         'RH'//rh(nrh)//'Bin'//BINFLAG(NB)(1:LENBFLAG(NB))//'Lam'
     +         //WAVEFLAG(NW)(1:LENWFLAG(NW))//'.mie.out'
c
c   Lecture des fichiers mie.out
c
                OPEN(UNIT=1,FILE=MIEFILE,STATUS='OLD')
                  READ(1,*) BIDON
                  READ(1,*) BIDON
                  READ(1,*) BIDON
                  DO 1111 NA=1,BNANG
                     READ(1,*) THETA(NA),PHAFCT(NA,NW,NB,NR,nrh)
 1111             CONTINUE
                  DO 1112 I=1,7
                     READ(1,*) BIDON
 1112             CONTINUE
C
C   LES SECTIONS EFFICACES SONT EN UNITES DE MICRONS^2
C 
                  READ(1,*) SIGMAE(NW,NB,NR)
                  READ(1,*) SIGMAS(NW,NB,NR)
                  READ(1,*) SIGMAA(NW,NB,NR)
               CLOSE(UNIT=1,STATUS='delete')
               if (NB.eq.1) then
                  WRITE(16,607) BINFLAG(NB)(1:LENBFLAG(NB)),
     +            SIGMAE(NW,NB,NR), SIGMAS(NW,NB,NR),SIGMAA(NW,NB,NR),
     +            'Bin number     ,   C,  Cs, Ca (um^2)'                   
               else
                  WRITE(16,607) BINFLAG(NB)(1:LENBFLAG(NB)),
     +         SIGMAE(NW,NB,NR), SIGMAS(NW,NB,NR),SIGMAA(NW,NB,NR),' '
               endif
 607           format(A9,1x,E12.5E2,1x,E12.5E2,1x,E12.5E2,T50,A)
 911        CONTINUE
 1011    CONTINUE
            print*,((NR-1)*13+nrh)*100/(NREF*13),'%'
 1009    continue
 811  CONTINUE
      close(unit=16)
c
c   ecriture du fichier de phase function
c
      nompfct=NOM(1:LENNOM)//'.pfc.bmi'
      print*,'Writing phase function output file: ',nompfct
      open(unit=17,file=nompfct,status='unknown')
      write(17,603) NREF,NWAV,NBNS
      do 801 NR=1,NREF
         WRITE(17,601)  NOMTYPE(NR)(1:LENTYPE(NR))
         do 805 nrh=1,13
            write(17,*) rh(nrh),'   Relative Humidity (%)'
         do 802 NW=1,NWAV
            WRITE(17,604) WAVELEN(NW),mr(NR,nrh,NW), mi(NR,nrh,NW)
            do 803 NB=1,NBNS
               WRITE(17,807) BINFLAG(NB)(1:LENBFLAG(NB))
 807          format(A6,T50,'Bin number')
                  DO 2111 NA=1,BNANG,5
                     if (NA.eq.1) then
                        write(17,809) THETA(NA),PHAFCT(NA,NW,NB,NR,nrh),
     +                  'Diffusion angle, Phase function'
                     else
                        write(17,809) THETA(NA),PHAFCT(NA,NW,NB,NR,nrh)
     +,' '
                     endif
 2111             CONTINUE
 809          format(F9.3,1x,E12.5E2,T50,A)
 803        continue
 802     continue
            print*,((NR-1)*13+nrh)*100/(NREF*13),'%'
 805     continue
 801  continue               
      close(unit=17)   
c
c   effacer les fichiers psd et .mie temporaires
c
      ...
      do 1131 NR=1,NREF
         do 1132 NW=1,NWAV
            do 1133 NB=1,NBNS
            do 1129 nrh=1,13
               mietemp=NOM(1:LENNOM)//NOMTYPE(NR)(1:LENTYPE(NR))//
     +         'RH'//rh(nrh)//'Bin'//BINFLAG(NB)(1:LENBFLAG(NB))//'Lam'
     +         //WAVEFLAG(NW)(1:LENWFLAG(NW))//'.mie'
               psdtemp=NOM(1:LENNOM)//NOMTYPE(NR)(1:LENTYPE(NR))//
     +         'RH'//rh(nrh)//'Bin'//BINFLAG(NB)(1:LENBFLAG(NB))//'Lam'
     +         //WAVEFLAG(NW)(1:LENWFLAG(NW))
               open(unit=26,file=psdtemp,status='old')
               close(unit=26,status='delete')
               open(unit=27,file=mietemp,status='old')
...
C    j ai generaliser le programme ce qui a permis de traiter le 
...
C    complementaires (MakeMieIn.f et MakePSD) permettent d editer les 
C    fichiers d entree.
...
C    normalisee sur toute la sphere.
C   -Dans la formulation du la PSD lognormale (programme MakePSD.f)
...
C
      SUBROUTINE MIES(RM,SG,FILE,NBINTER,INTERAC)
C
C ----------
C
C   DECLARATION DES VARIABLES
C
      ...
      REAL*8 PI, BINDENSITY(5002),DX,DY,NBINTER,XI,SG,XM,RM
      COMPLEX*8 Z,M1,DUMC8(3003)
      COMPLEX*16 M,CM,DUMC16(17500)
      CHARACTER*60 , INFILE, OUTFILE, PSDFILE, FILE
      INTEGER INFILELENGTH, NBINS, II, III, IMPRIMER, PSDFILELENGTH
      INTEGER bncount(5002), bncmax, interp, INTERAC, POL,ITYPE
      COMMON /PHASE/ RI
...
C
 2001  PRINT*,'RACINE DU NOM DES FICHIERS ENTREE-SORTIE ?(MAX 40 CARACTE
     +RES):'
       PRINT*,'Les extensions .mie (fichier d entree) et .mie.out (sorti
     +e)'
       PRINT*,'seront ajoutees a la racine.'
...
C
       IF (IMPRIMER.EQ.1) THEN
            PRINT*,'Ouverture du fichier de sortie:',OUTFILE(1:
     +INFILELENGTH+8),'...'
       ENDIF
...
C 
       IF (IMPRIMER.EQ.1) THEN
            PRINT*,'Lecture du fichier psd:',PSDFILE(1:PSDFILELENGTH),'.
     +..'
       ENDIF     
       READ(2,*) IPSD
       READ(2,*) RINC
...
C
       ...
       ENDIF
c
c   condition pour le cas discret seul le premier sous bin sera consi-
c   dere.
c
       if (interp.eq.9) then
             bncmax=1
       ...
       IF (INTERAC.EQ.1) THEN
          print*
          print*
 34       PRINT*,'EN COMBIEN DE SOUS-BINS VOULEZ-VOUS DIVISER LA PSD? (M
     +AX. 5000., MIN',NBINS,')'
          READ*,NBINTER
...
C
       IF (IMPRIMER.EQ.1) THEN
            PRINT*,'Fermeture du fichier psd.'
       ENDIF     
       CLOSE(UNIT=2)
C ----------
C
...
C  
      READ(1,*) ITYPE
      IF (IMPRIMER.EQ.1) THEN
           PRINT*,'Forme de particule:'
           IF (ITYPE.EQ.1) THEN
                WRITE(*,3004)
           ELSEIF (ITYPE.EQ.2) THEN
                WRITE(*,3001)
           ELSE
                PRINT*,'Type d aerosol invalide, Executez MakeMieIn.'
           ENDIF
      ENDIF
C
C -----------------
...
C
      IF (IMPRIMER.EQ.1) THEN
            PRINT*,'Determination de l intervalle et du parametre de tai
     +lle...'
      ENDIF
...
C
                IF (IMPRIMER.EQ.1) THEN
                     PRINT*,'Definition de la distribution de taille...'
                ENDIF
...
c
     ...
     +  BEGINBIN(II))
                               ...
                               if (interp.eq.2) then
c
c   les valeurs de XM (X moyen) et SG (sigma) sont choisies pour etre 
c   representative d une PSD Moyenne pour le type de particules.  
c   Ces valeurs peuvent etre
c   modifiees pour optimiser la ponderation.  Il faut toutefois voir 
c   ce calcul comme une correction de premier ordre.  Il y aura toujours
c   des difference entre un calcul detaille ou la PSD est connue et un 
c   calcul global par bin ou la psd a l interieur du bin est inconnue
c
                                 XM=2.*PI*RM/WAVLEN
                         PSD(I)=BINDENSITY(II)*(1./SG/XI/R2PI)
     +                   *EXP(-((LOG(XI)-LOG(XM))**2./2./SG**2.)) 
                               endif
                               if (interp.eq.3) then
c
c  moment 0 pour l integration selon log(X).  La division par 1/X 
c  permet de tenir compte du fait que la routine mies integre selon
c  la variable dX et non dlog(X)
c
                               endif
                                  PSD(I)=BINDENSITY(II)/XI
                               if (interp.eq.4) then
c
c  moment 1 pour l integration selon log(X).  La division par 1/X 
c  permet de tenir compte du fait que la routine mies integre selon
c  la variable dX et non dlog(X)
c

                                  PSD(I)=BINDENSITY(II)*dabs(log(XI))/XI
                               endif
...
C
                IF (IMPRIMER.EQ.1) THEN
                     PRINT*,'Calcul des proprietes optiques...'
                ENDIF
                READ(1,*) ANGLEL,ANGLEU,DELTA
                IF(DELTA.LT.0.01) THEN
                     IF (IMPRIMER.EQ.1) THEN
                          WRITE(*,1042)
                          PRINT*,'Increment angulaire trop petit (<0.01)
     + Executez MakeMieIn a nouveau.'
                     ENDIF
                     STOP
                ENDIF
                READ(1,*) POL
                IF (IMPRIMER.EQ.1) THEN
                     ...
                     ELSE
                          PRINT*,'Mauvais etat de polarisation: Executez 
     +MakeMieIn a nouveau'
                     ENDIF
                ENDIF
                IF(POL.NE.2.) IPARA=1
...
C
     ...
     +WRITE(*,1102)
                ...
                ENDIF
                IF (IMPRIMER.EQ.1) THEN
                     PRINT*,'Fermerture des fichiers entree-sortie...'
                ENDIF
C
C ----------
C   
...
C
           ...
           IF (IMPRIMER.EQ.1) THEN
                PRINT*,' '
                PRINT*,'Les resultats se trouvent dans le fichier: ',OUT
     +FILE(1:INFILELENGTH+8)
                PRINT*,'...Fin normale de mies!'
           ENDIF
           return
           end
C
...
C
 ...
 1306 FORMAT(1X,'THERE ARE NO PARTICLES IN THIS SIZE RANGE a',f8.5)
      RETURN
      END
C
C
...
C
 1006 FORMAT(G12.6,' = Lidar ratio')
 1306 FORMAT(1X,'THERE ARE NO PARTICLES IN THIS SIZE RANGE b',F8.5)
      RETURN
      END
