...
c  des matrices entrainent l usage d environ 320 Mb de RAM!
c
c  copyright Martin Aube 2002
c
c ---------------------------------------------------------------------
c
      program dynamic
...
c  tt_3d est la matrice de temperature sur la grille 3d du modele
     ...
     +     vx(ntyp,nbins,nxwid,nywid,nzlev),
     +     vxp(ntyp,nbins,nxwid,nywid,nzlev),vyp(ntyp,nbins,nxwid,
     +     nywid,nzlev),vzp(ntyp,nbins,nxwid,nywid,nzlev),
     +     vy(ntyp,nbins,nxwid,nywid,nzlev),vz(ntyp,nbins,nxwid,
     +     nywid,nzlev),ax,depot(ntyp,nbins,nxwid,nywid),
     +     ay,az,vlz,vlimz,ratx(3),raty(3),ratz(3),wetrem(ntyp,
     +     nbins,nxwid,nywid),
...
c
     ...
     +       rhtim,rhtim2,ordon,pente,dtminm,wstim,wstim2,
     +       wetim,wetim2,srctim,srctim2
c
...
c
     ...
     +             bidon,nomsrc(1000),
     +             rhfile2,wsfile2,wetfile2,srcfile2
          ...
          do 42 ic=1,nxwid 
            do 43 id=1,nywid
              ...
              do 44 ie=1,nzlev
                ...
                vx(ia,ib,ic,id,ie)=0.
                vy(ia,ib,ic,id,ie)=0.
                vzp(ia,ib,ic,id,ie)=0.
                vxp(ia,ib,ic,id,ie)=0.
                vyp(ia,ib,ic,id,ie)=0.
 44           continue
 43         continue 
 42       continue
 41     continue
...
c
     ...
     +      .ne.srctim)) then
          ...
          open(unit=20,file=srcfile,status='old',err=22)
            print*,'Reading emission file: ',srcfile(1:19)
            read(20,*) bidon
            read(20,*) bidon
            read(20,*) hre,min,sec,jou,moi,ann
            read(20,*) ival
...
c
     ...
     +              i=ncellx,1,-1)
          ...
          open(unit=20,file=srcfile2,status='old',err=22)
            print*,'Reading emission file: ',srcfile2(1:19)
            read(20,*) bidon
            read(20,*) bidon
            read(20,*) hre,min,sec,jou,moi,ann
            read(20,*) ival
...
c
        ...
        if (t.eq.1) then
c
c  initial velocity and position
c  velocity = wind velocity
c
          do 650 nt=1,ntype
            do 660 nb=1,nbns
              do 670 nx=1,ncellx
                do 680 ny=1,ncelly
                  do 690 nz=1,ncellz
                    vx(nt,nb,nx,ny,nz)=vxwind(nx,ny,nz)
                    vy(nt,nb,nx,ny,nz)=vywind(nx,ny,nz)
                    vz(nt,nb,nx,ny,nz)=vzwind(nx,ny,nz)
 690              continue
 680            continue
 670          continue
 660        continue
              ...
              do 720 nx=1,ncellx
                do 730 ny=1,ncelly
                  do 740 nz=1,ncellz
                    ...
                    vx(nt,nb,nx,ny,nz)=vxp(nt,nb,nx,ny,nz)
                    vy(nt,nb,nx,ny,nz)=vyp(nt,nb,nx,ny,nz)
                    vz(nt,nb,nx,ny,nz)=vzp(nt,nb,nx,ny,nz)
                    if (vxp(nt,nb,nx,ny,nz).eq.0.) then
                      vx(nt,nb,nx,ny,nz)=vxwind(nx,ny,nz)
                    endif
                    if (vyp(nt,nb,nx,ny,nz).eq.0.) then
                      vy(nt,nb,nx,ny,nz)=vywind(nx,ny,nz)
                    endif
                    if (vzp(nt,nb,nx,ny,nz).eq.0.) then
                      vz(nt,nb,nx,ny,nz)=vzwind(nx,ny,nz)
                    endif
 740              continue
 730            continue
               ...
               do nt=1,ntype              
                 ...
                 do nb1=1,nbns
                  if (numv(nb1).ne.0.) then
                   dn_dt(nb1)=dn_dt(nb1)-0.5*numv(nb1)**2.*kcoag(nz,
     +             ntmp,nrh,nt,nb1,nb1)
                   if (nb1.ne.1) then
                     dn_dt(nb1)=dn_dt(nb1)+0.5*numv(nb1-1)**2.*
     +               kcoag(nz,ntmp,nrh,nt,nb1-1,nb1-1)  
                   endif
                   if (nb1.ne.nbns)  then                       
                     do nb2=nb1+1,nbns    
...
c
     ...
     +              viscos/cc
c
c  determining local acceleration of particle from 2nd 
c  Newton s law and computing limit speed
c   
c  gravitation
c  friction
c  Archimede s force
c
                    vx0=vx(nt,nb,nx,ny,nz)
                    vy0=vy(nt,nb,nx,ny,nz)
                    vz0=vz(nt,nb,nx,ny,nz)
                    ax=-b*(vx(nt,nb,nx,ny,nz)-vxwind(nx,ny,nz))/masse  
                    vlx=ax*(deltat*24.*3600.)+vx0
                    ay=-b*(vy(nt,nb,nx,ny,nz)-vywind(nx,ny,nz))/masse 
                    vly=ay*(deltat*24.*3600.)+vy0
                    az=-b*(vz(nt,nb,nx,ny,nz)-vzwind(nx,ny,nz))/masse+
     +              g*(rhoair/finden(ncr(nt),nrh,nb)-1.) 
                    vlz=az*(deltat*24.*3600.)+vz0
...
c
                    if ((az.gt.0.).and.(vlz.gt.vlimz)) then
                      az=0.
                      vz0=vlimz
                    endif
                    if ((az.le.0.).and.(vlz.lt.vlimz)) then
                      az=0.
                      vz0=vlimz
                    endif
c
...
c
                    if ((ax.gt.0.).and.(vlx.gt.vxwind(nx,ny,nz))) then
                      ax=0.
                      vx0=vxwind(nx,ny,nz)
                    endif
                    if ((ax.lt.0.).and.(vlx.lt.vxwind(nx,ny,nz))) then
                      ax=0.
                      vx0=vxwind(nx,ny,nz)
                    endif
                    if ((ay.gt.0.).and.(vly.gt.vywind(nx,ny,nz))) then
                      ay=0.
                      vy0=vywind(nx,ny,nz)
                    endif
                    if ((ay.lt.0.).and.(vly.lt.vywind(nx,ny,nz))) then
                      ay=0.
                      vy0=vywind(nx,ny,nz)
                    endif
c
c  determining next particle position
c
                    x=0.5*ax*(deltat*24.*3600.)**2.+vx0*deltat*24.
     +              *3600.
                    y=0.5*ay*(deltat*24.*3600.)**2.+vy0*deltat*24.
     +              *3600.
                    z=0.5*az*(deltat*24.*3600.)**2.+vz0*deltat*24.
     +              *3600.+z
c
...
c
 ...
 900                continue  
c
c  computing new speed of cell
c
                   if ((newnx.ge.1).and.(newny.ge.1).and.(newnz.ge.1))
     +             then
                    vxp(nt,nb,newnx,newny,newnz)=ax*deltat*24.*3600.+
     +              vx0
                    vyp(nt,nb,newnx,newny,newnz)=ay*deltat*24.*3600.+
     +              vy0
                    vzp(nt,nb,newnx,newny,newnz)=az*deltat*24.*3600.+
     +              vz0
                   endif
                  endif
 810            continue
...
c
 ...
 9    print*,'Aerosol dist. file too big (max ',nxwid,'x',
     +nywid,'x',nzlev,')!'
      stop
 14   print*,'File climato.dat not found! Please run mkcli.'
      stop
 ...
 22   print*,'Source file not found! Please run mksrc.'
      stop
 23   print*,'Nb of size bins in climato.dat don t match run prop.'
      stop
 24   print*,'Nb of cells in climato.dat don t match run properties'
      stop
 25   print*,'Cell size in climato.dat don t match run properties'
      stop
 26   print*,'Nb of vert. cells in climato.dat don t match run prop.'
      stop
 27   print*,'Nb of aerosol types in climato.dat don t match run prop.'
      stop
...
c
        ...
        character*60 rhfile  
        print*,'Reading humidity file: ',rhfile(1:15)
...
c
        ...
        character*60 wsfile
        print*,'Reading wind speed file: ',wsfile(1:15)
...
c
        ...
        print*,'Reading precipitation file: ',wetfile(1:15)
...
c
        ...
        character*60 ttfile
        print*,'Reading air temperature file: ',ttfile(1:15)
...
c
     ...
     +          lcellz(k)
      ...
      return
      stop
 2314   print*,'File climato.dat not found! Please run mkcli.'
      stop
