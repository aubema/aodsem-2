c    programme pour interpoler et combiner jusqu'a 4 cartes d epaisseur optique
c    sous echantillonnees a partir de quelques mesures ponctuelles 
c    effectuees par des photometres solaires et/ou par inversion d images 
c    de teledetection.
c
...
c    maximum ............. 5
c
c  Une correction de voisins immédiats pour la correlation spatiale des
c  donnees est effectuee. De plus l'interpolation est faite un inverse du carre
c  de l'erreur associee a la mesure.
c    
...
c   65533=lignes cotieres, autre=aod*1000
c
       do n=1,nbdat
...
c     
              ...
              do j=1,ncelly
                do nn=1,10
                   dens(n,nn,i,j)=0
                enddo 
               if (tau(n,i,j).lt.65.533) dens(n,9,i,j)=1
            if (j.ne.ncelly) then 
               if (tau(n,i,j+1).lt.65.533) dens(n,1,i,j)=1
            elseif ((i.ne.ncellx).and.(j.ne.ncelly)) then
               if (tau(n,i+1,j+1).lt.65.533) dens(n,2,i,j)=1
            elseif (i.ne.ncelly) then
               if (tau(n,i+1,j).lt.65.533) dens(n,3,i,j)=1 
            elseif ((i.ne.ncellx).and.(j.ne.1)) then               
               if (tau(n,i+1,j-1).lt.65.533) dens(n,4,i,j)=1
            elseif (j.ne.1) then            
               if (tau(n,i,j-1).lt.65.533) dens(n,5,i,j)=1 
            elseif ((i.ne.1).and.(j.ne.1)) then 
               if (tau(n,i-1,j-1).lt.65.533) dens(n,6,i,j)=1 
            elseif (i.ne.1) then               
               if (tau(n,i-1,j).lt.65.533) dens(n,7,i,j)=1
            elseif ((i.ne.1).and.(j.ne.ncelly)) then                
                if (tau(n,i-1,j+1).lt.65.533) dens(n,8,i,j)=1  
            endif
...
c    
     ...
     +           real(i))**2.)
                ...
                if (distan.le.real(side)) then
                 if (distan.lt.0.01) distan=1.
c la condition precedente a pour effet de produire une forme de fonction de poids 
c top hat qui permet l'influence d'une valeur ponctuelle comme aeronet situee a proximite
c d'une autre mesure. Auparavant la distan etait tres petit ce qui faisant en sorte que seul 
c la valeur a la position courrante avait un impact significatif.
                 if ((i.gt.0).and.(i.le.ncellx)) then
                    if ((j.gt.0).and.(j.le.ncelly)) then
                       if (tau(n,i,j).le.65.532) then
c
c nous combinerons les donnees en proportion inverse de leur variance d'erreur
c
     ...
     +                       tau(n,i,j)+offset(n))**2.)+tauout(nx,ny) 
                             dtotal=1./(distan*(real(dens(n,10,i,j))))/
     +                       ((gain(n)*tau(n,i,j)+offset(n))**2.)+
     +                       dtotal  
...
c                      factor=factor+8.*pixsiz
              ...
              if (tauout(nx,ny).ge.65.532) then
                 tauout(nx,ny)=65.534
                 if (autom.eq.1) then
c                   factor=factor+8.*pixsiz
                    factor=factor*2.
...
c
     ...
     +erp.f, (1000=aod 1)'
         ...
         do 3411 i=ncellx,1,-1
               write(27,*) (nint(tauout(i,j)*1000.),j=1,ncelly)
 3411    continue
 177     format(i3)
 178     format(A)
 179     format(A,F12.6)
