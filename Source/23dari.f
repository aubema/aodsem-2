c    Programme de Jean-Denis Giguère d'après le programme de Martin Aubé 3dari

c    Programme permettant proceder a des operations arithmetiques de 
c    base sur une distribution 3D d'après les informations contenus
c    dans un fichier .pgm
c
c    copyright martin aube 2001
c    et jean-denis giguere 2003
c
c -----------------
...
c
       character*60 nom1,nom2,nom3,datfil1,pgmfil,datfil3,
     + obsfile,nomtype(5),binflag(12)
       character iscomment
       real numvol1(5,12,125,275,10),pgmval(125,275),
     + numvol3(5,12,125,275,10),rayon1(12),rayon2(12),lcellx
     + ,lcelly,lcellz(10),xcell0,ycell0,zcell0,discardf
       real heure,minute,seconde,jour,mois,annee
       integer lennom1,lennom2,lennom3,nr,nb,i,j,k,oper,neg
     + ,nbns,lenbflag(12), size_i, size_f, alti_i, alti_f, 
     + type_i, type_f, vf,vy
     + ,ncellx,ncelly,ncellz,lentype(5),nref,discardi,nNS,nEO
c   
c ----------
c
...
c
      open(unit=25,file='23dar.par',status='old',err=8)
       read(25,*) nom1
       read(25,*) nom2
...
c      print*,'   4 ..... /'
       read(25,*) oper
       read(25,*) type_i, type_f
       read(25,*) size_i, size_f
       read(25,*) alti_i, alti_f
       read(25,*) nom3
       read(25,*) neg
      close(unit=25)
c
...
c
          print*,'Reading data file: ',datfil1(1:18)
          read(20,*) 
          read(20,*) 
...
c ___________
c
c   lecutre du fichier domain.par
c
      open(unit=25,file='domain.par',status='old',err=11)
        read(25,*) discardi, nEO, nNS
      close(unit=25)

c
c __________
c
c  lecture du fichier pgm
c
      open(unit=30,file=pgmfil,status='old',err=4)
      read(30,*) iscomment
      read(30,*) iscomment
c si la ligne est un commentaire, lire la ligne suivante
      do  while (iscomment(1:1) .eq. '#' )
       read(30,*) iscomment
      enddo
      read(30,*) iscomment
      do i=nEO,1,-1
      read(30,*) (pgmval(i,vy),vy=1,nNs)
      enddo
...
c
       ...
       do nr=type_i,type_f
         print*,(nr-type_i)*100/(type_f-type_i+1),'%'
         do nb=size_i,size_f
           do i=1,ncellx
             do j=1,ncelly
               do k=alti_i,alti_f
c         somme
               if (pgmval(i,j).lt.253.) then
                 if (oper.eq.1) then
...
c
 ...
 3     print*,'Bad root file name: ',nom1
       stop
 4     print*,'Bad root file name: ',nom2
       stop
 8     print*,'Parameter file: 3dtarit.par dont exist.'
       stop
 11    print*,'domain.par does not exit, run domai.'
       stop
       end
