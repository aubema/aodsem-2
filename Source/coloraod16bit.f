c  Version 16 bit du code coloraod.f
c  la valeur maximale est passee de 2.52 à 65.532 
c  Cette version doit etre utilisee avec les pgm produits
c  avec le autre codes 16 bits
c  la variable top permet de fixer la limite de l'aod en terme de
c  la plage du code de couleur si l'aod > top alors la couleur sera 
c  blanche
c  si aod=top alors ce sera rouge
c
c ====================
c
...
c
c
c    copyright martin aube 26/04/2004
...
c
      ...
      real pmasky,latit,longi,val,scale,top,sig
...
c
 ...
 55      continue
         ...
         do 5001 i=1,ncellx
            do 5002 j=1,ncelly
               tau(i,j)=scale*tau(i,j)/1000.

               if (tau(i,j).gt.65.533) tau(i,j)=65.534
           ...
           do 2311 j=1,ncelly
              ...
              if ((i.gt.bufw).and.(i.le.ncellx-bufw)) then
               if ((j.gt.bufw).and.(j.le.ncelly-bufw)) then   
                ...
                if (tau(i,j).gt.top) then               
                   r=65534
                   g=65534
                   b=65534
                   if (nint(tau(i,j)*1000.).eq.65535) then
                     r=65535
                     g=65535
                     b=65535
                   endif
                ...
                elseif (nint(tau(i,j)*real(ncolor)).lt.0) then 
                 ...
                 r=nint(65535.*exp(-((tau(i,j)-(0.70*top))/
     +           (1.*sig))**2.))
                 if (tau(i,j).gt.0.70*top) r=65535
                 if (tau(i,j).le.0.30*top) then
                   g=nint(65535.*exp(-((tau(i,j)-0.30*top)/
     +             (0.8*sig))**2.))
                 elseif (tau(i,j).ge.0.7*top) then
                   g=nint(65535.*exp(-((tau(i,j)-.7*top)/
     +             (0.8*sig))**2.))
                 else
                   g=65535
                 endif
                 if (tau(i,j).lt.0.29*top) then
                   b=nint(65535.*exp(-((tau(i,j)-0.29*top)/
     +            (1.55*sig))**2.))
                 else
                   b=nint(65535.*exp(-((tau(i,j)-0.29*top)/
     +            (1.3*sig))**2.))
