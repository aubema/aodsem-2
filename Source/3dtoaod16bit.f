c   Version 16 bit de la version originale 8 bit du programme 
c   3dtoaod.f
c
c   impact de cette mise a jour: le gain pour passer a l'épaisseur
c   optique sera de 1/1000 au lieu de 1/100 et la valeur maximale
c   qui etait de 2,52 passera a 65,532
c
c ================================
c
...
c
          print*,'Reading observing geometry file: ',visfile(1:20)
          read(7,*)
          read(7,*) nobs
          do 108 no=1,nobs
             read(7,*) elepho(no)
...
c
          print*,'Reading data file: ',datfile(1:20)
          read(20,*) bidon
          read(20,*) bidon
            ...
            do 1811 i=ncxi,ncxf
               do 1911 j=ncyi,ncyf
                  do 2011 k=nczi,nczf
                     ...
                     if (taumap(nw,i,j).gt.65.532) taumap(nw,i,j)=
     +               65.534
 2011             continue
 1911          continue
 1811       continue
...
c ------------------
c
c   fabrication des images pgm (valeur = epaisseur optique * 1000)
c
...
c                  if (taumap(nw,i,j).gt.65.535) taumap(nw,i,j)=65.535
                  write(27,*) (nint(taumap(nw,i,j)*1000.),j=1,ncelly)
 3411       continue
         elseif (axis(ns).eq.2) then 
            write(27,172) ncellx, ncellz
            write(27,*) '65535'
            do 3412 k=ncellz,1,-1
               write(27,*) (nint(taumap(nw,i,k)*1000.),i=1,ncellx)           
 3412       continue
         elseif (axis(ns).eq.1) then 
            write(27,172) ncelly, ncellz
            write(27,*) '65535'
            do 3413 k=ncellz,1,-1
               write(27,*) (nint(taumap(nw,j,k)*1000.),j=1,ncelly)           
 3413       continue
         endif
 177     format(i3)
         close(unit=27)
 ...
 171  format(a)
 172  format(i6,1x,i6)
 174  format('# AOD map from AODSEM (1000= aod 1)')
