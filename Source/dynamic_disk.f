...
c  tt_3d est la matrice de temperature sur la grille 3d du modele
     ...
     +     ,vzp,
     +     vy,vz
     +     ,ax,depot(ntyp,nbins,nxwid,nywid),
...
c
      ...
      integer nttfi,nb1,nb2,bido1,ntmp,nbuffer,nbu,posi
...
c
       open(unit=30,file='vxp.tmp',status='unknown',recl=4,
     + access='direct')
       open(unit=31,file='vyp.tmp',status='unknown',recl=4,
     + access='direct')	
       open(unit=32,file='vzp.tmp',status='unknown',recl=4,
     + access='direct')
       open(unit=33,file='vx.tmp',status='unknown',recl=4,
     + access='direct')
       open(unit=34,file='vy.tmp',status='unknown',recl=4,
     + access='direct')
       open(unit=35,file='vz.tmp',status='unknown',recl=4,
     + access='direct')
c
c  remplir de valeurs nulles
c
          print*,'Initializing swap files...'
          vx=0.
          vy=0.
          vz=0.
          vxp=0.
          ...
          do nt=1,ntype
            do nb=1,nbns
              do nx=1,ncellx
                do ny=1,ncelly
                  do nz=1,ncellz
                    posi=(nt-1)*nbns*ncellx*ncelly*ncellz+(nb-1)*
     +              ncellx*ncelly*ncellz+(nx-1)*ncelly*ncellz+(ny-1)*
     +              ncellz+nz
                    write(30,rec=posi) vxp
                    write(31,rec=posi) vyp
                    write(32,rec=posi) vzp
                    write(33,rec=posi) vx
                    write(34,rec=posi) vy
                    write(35,rec=posi) vz
                  enddo
                enddo
              enddo
            enddo
...
c
 ...
 650      continue
     ...
     +              ncellx*ncelly*ncellz+(nx-1)*ncelly*ncellz+(ny-1)*
     +              ncellz+nz
                    read(30,rec=posi) vxp
                    read(31,rec=posi) vyp
                    read(32,rec=posi) vzp
                    vx=vxp
                    vy=vyp
                    vz=vzp
                    if (vxp.eq.0.) then
...
c
     ...
     +              ncellz+nz
                    ...
                    vy0=vy
                    vz0=vz
                    ax=-b*(vx-vxwind(nx,ny,nz))/masse  
                    vlx=ax*(deltat*24.*3600.)+vx0
                    ay=-b*(vy-vywind(nx,ny,nz))/masse 
                    vly=ay*(deltat*24.*3600.)+vy0
                    az=-b*(vz-vzwind(nx,ny,nz))/masse+
...
c
     ...
     +             then
                    posi=(nt-1)*nbns*ncellx*ncelly*ncellz+(nb-1)*
     +              ncellx*ncelly*ncellz+(newnx-1)*ncelly*ncellz+
     +              (newny-1)*ncellz+newnz          
                    vxp=ax*deltat*24.*3600.+
     +              vx0
                    vyp=ay*deltat*24.*3600.+
     +              vy0
                    vzp=az*deltat*24.*3600.+
     +              vz0
                    write(30,rec=posi) vxp
                    write(31,rec=posi) vyp
                    write(32,rec=posi) vzp
