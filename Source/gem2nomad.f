...
c   applicable pour les donnees traduite par nomad a partir
c   du site http://www.weatheroffice.ec.gc.ca/grib/High-resolution_GRIB_f.html
c
c
       program gem2nomad
c
...
c
       ...
       integer nf,plevel(12),nl,k,i,j,RH(181,361,10),ncellz
       integer lenom,hre,min,sec,jou,moi,ann,prswitch,ptime,nfil,nfi
       integer bid,hre2,nhre(200),pr00s,pr06s,pr12s,joutmp
	   integer aan,mmo,jjo,hhe,delay
       real varp(200)
       real donnee(181,361,12),lcellx,lcelly,xcell0,ycell0,hcellz(10)
       real P(181,361,10),h0,P0,dP_dz(181,361,10),dP_dx(181,361,10),m,b
       real dP_dy(181,361,10),dx,dy,latt,VV(181,361,10),UU(181,361,10)
       real vz(181,361,10),dP_dt(181,361,10),dt,P2(181,361,10)
       real TT(181,361,10),ea,eta,ES(181,361,10),pi,PR06(181,361)
       real PR00(181,361),PR12(181,361),lcellz(10)
...
c   niveaux de pression du GEM
       data plevel /1000,925,850,700,500,400,300,250,200,150,100,50/
...
c   Donnees d entree
c    
c       print*,'Time step between two GEM files (hour)?'
c       read*,dt  
       dt=6. 
c conversion en secondes
	   dt=dt*3600.
       open(unit=7,file='gem.index',status='old')
          ...
          do 100 nfi=1,nfil-1
             ...
             backspace 7
			 read(gemfi1,229) aan,mmo,jjo,hhe,bidon4,delay
 229   format (I4,I2,I2,I2,A2,I2)
...
c
 ...
 110   enddo  
c
c test particulier aux fichiers meteo utilises par nomad (60 fields pour les analyses et 1 pour les precipitations)
c       
	   if ((nfield.eq.1).or.(nfield.eq.60)) then
         rewind 2
	   else
	     print*,'******* Bad data file, skipping file: ',gemfi1
	     goto 100
	   endif
       nf=0
       do n=1,nlign
...
c			  varp(nf)=delay
             ...
             do nn=1,5
               read(2,*)
             enddo 
c             read(2,*) bidon1,bidon2,varp(nf)
              read(2,*)
			  varp(nf)=delay
...
c
       ...
       if (prswitch.eq.0) then
c
c   Lecture des niveaux GZ et chargement dans la matrice donnee
c
c   dans la lecture des donnees, y et j donne la position le long de la ligne du fichier
c   (de la gauche vers la droite, i.e. de l'ouest vers l'est)
c   alors que x et i donnent la position dans la colonne
c   (du haut vers le bas, i.e. du nord au sud)
c
...
c       do i=1,ncellx
c       do j=1,ncelly
c         donnee(i,j,nl)=donnee(i,j,nl)*10.
c       enddo
c       enddo
c       enddo
c  
...
c 
     ...
     +       /real(plevel(2)))
           ...
           if (hcellz(k).gt.donnee(i,j,12)) then
             h0=(donnee(i,j,12)-donnee(i,j,11))/log(real(plevel(11))
     +       /real(plevel(12)))
             if (h0.eq.0.) goto 42
             P0=real(plevel(12))/(exp(-donnee(i,j,12)/h0)) 
             P2(i,j,k)=P0*exp(-hcellz(k)/h0)  
           endif 		     
        enddo   
       enddo
         ...
         if (bidon(1:8).eq.'variable') then
           ...
           if (varn(nf).ne.'PR') then
             do nn=1,3
               read(1,*)
             enddo
c            read(1,1111) bido,time(nf)
             read(1,*)
             time(nf)=dble(hhe)
             nhre(nf)=nint(100.*(time(nf)-real(int(time(nf)))))
c 1111        format(1x,A9,D14.3)
             do nn=1,4
               read(1,*)
             enddo          
...
c             read(1,1111) bido,time(nf)
             ...
             nhre(nf)=nint(100.*(time(nf)-real(int(time(nf)))))
             read(1,*) 
c             read(1,*) bidon1,bidon2,varp(nf)
              varp(nf)=delay
...
c
       ...
       rewind 1
c
c   Lecture de la precipitation (unite=m/6 hre) mais initialement kg/m**2 /6 hre (PR)
c
...
c conversion d'unites en divisant par le densite de l'eau de 1000 kg/m**3	   
...
c 
     ...
     +         /real(plevel(nl+1))) 
			 ...
			 if (	dP_dz(i,j,k).eq.0.) then
			 print*,"cas1", P(i,j,k),h0,P0,donnee(i,j,nl+1),donnee(i,j,nl)
			 stop
			 endif			 
     ...
     +       /real(plevel(2)))
			 ...
			 if (	dP_dz(i,j,k).eq.0.) then
			 print*,"cas2", P(i,j,k),h0,P0,donnee(i,j,2),donnee(i,j,1)
			 stop
			 endif			 
     ...
     +       /real(plevel(12)))
			 ...
			 if (	dP_dz(i,j,k).eq.0.) then
			 print*,"cas3", P(i,j,k),h0,P0,donnee(i,j,12),donnee(i,j,11)
			 stop
			 endif
...
c       do i=1,ncellx
c       do j=1,ncelly
c         donnee(i,j,nl)=donnee(i,j,nl)/1.94384
c       enddo
c       enddo
c       enddo  
c
...
c   
       print*,'Projection de VV sur la grille'     
       do i=1,ncellx
       do j=1,ncelly
       do k=1,10
         do nl=1,11
           if ((real(plevel(nl)).ge.P(i,j,k)).and.(real(plevel(nl+1))
     +     .le.P(i,j,k))) then
             m=(donnee(i,j,nl+1)-donnee(i,j,nl))/(real(plevel(nl+1)
     +       -plevel(nl)))
             b=donnee(i,j,nl)-m*real(plevel(nl))
             VV(i,j,k)=m*P(i,j,k)+b
           elseif (P(i,j,k).gt.real(plevel(1))) then
             VV(i,j,k)=donnee(i,j,1)
           elseif (P(i,j,k).lt.real(plevel(12))) then               
           VV(i,j,k)=donnee(i,j,12) 
           endif
         enddo
       enddo
       enddo
...
c       
     ...
     +        -plevel(nl)))
             ...
             ES(i,j,k)=m*P(i,j,k)+b
	        	  ...
	        	  if (ES(i,j,k).lt.0.) then
	    print*,'ES prob 0',i,j,k,ES(i,j,k),P(i,j,k),m,b,
     +  donnee(i,j,nl),donnee(i,j,nl+1)
	    if (abs(ES(i,j,k)).lt.0.001) then 
		   ES(i,j,k)=0.
		else
		   print*,'Error ES<0'
	       stop
           ...
           elseif (P(i,j,k).gt.real(plevel(1))) then
             ES(i,j,k)=donnee(i,j,1)
	         	  ...
	         	  if (ES(i,j,k).lt.0.) then
	    print*,'ES prob 1',i,j,k,ES(i,j,k),P(i,j,k),m,b,donnee(i,j,1)
		          endif		  	
			 
			 
           elseif (P(i,j,k).lt.real(plevel(12))) then               
             ES(i,j,k)=donnee(i,j,12) 
			 
		          if (ES(i,j,k).lt.0.) then
		 print*,'ES prob 2',i,j,k,ES(i,j,k),P(i,j,k),m,b,donnee(i,j,12)
		          endif		  			 
...
c
c si en kelvin, convertir en deg. C on suppose que si TT>100 alors nous  sommes en kelvin
c       
             do j=1,ncelly
              do i=1,ncellx
                if (donnee(i,j,nl).gt.100.) then
                  donnee(i,j,nl)=donnee(i,j,nl)-273.15
		endif
              enddo
             enddo
           enddo
...
c pour les equations, se referer a Guyot Climatologie de d'environnement ,Dunod 1999
c 
...
c
     ...
     + sec,jou,moi,ann
       ...
       real vxwind(181,361,10),vywind(181,361,10),vzwind(181,361,10)
       character*60 wsfile
       zcell0=0.
...
c
     ...
     + lcellz,xcell0,ycell0,precip,ptime,jou,moi,ann)
       ...
       real lcellz(10),lcellx,lcelly,xcell0,ycell0,precip(181,361)
...
c
     ...
     + sec,jou,moi,ann
       integer ncelly,ncellz,t_drh(181,361,10)
       real lcellz(10),lcellx,lcelly,xcell0,ycell0,zcell0   
       character*60 rhfile  
       zcell0=0.
...
c
     ...
     + sec,jou,moi,ann
       integer ncelly,ncellz
       real lcellz(10),lcellx,lcelly,xcell0,ycell0,zcell0,tt(181,361,10)   
       character*60 ttfile  
       zcell0=0.
