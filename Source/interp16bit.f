...
c             
              ...
              do j=1,nytcel
                do n=1,10
                   dens(n,i,j)=0
                enddo 
               if (tau(i,j).lt.65.533) dens(9,i,j)=1
            if (j.ne.ncelly) then 
               if (tau(i,j+1).lt.65.533) dens(1,i,j)=1
            elseif ((i.ne.ncellx).and.(j.ne.ncelly)) then
               if (tau(i+1,j+1).lt.65.533) dens(2,i,j)=1
            elseif (i.ne.ncelly) then
               if (tau(i+1,j).lt.65.533) dens(3,i,j)=1 
            elseif ((i.ne.ncellx).and.(j.ne.1)) then               
               if (tau(i+1,j-1).lt.65.533) dens(4,i,j)=1
            elseif (j.ne.1) then            
               if (tau(i,j-1).lt.65.533) dens(5,i,j)=1 
            elseif ((i.ne.1).and.(j.ne.1)) then 
               if (tau(i-1,j-1).lt.65.533) dens(6,i,j)=1 
            elseif (i.ne.1) then               
               if (tau(i-1,j).lt.65.533) dens(7,i,j)=1
            elseif ((i.ne.1).and.(j.ne.ncelly)) then                
                if (tau(i-1,j+1).lt.65.533) dens(8,i,j)=1  
            endif
