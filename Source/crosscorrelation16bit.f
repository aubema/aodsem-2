...
c    les valeurs superieures ou egales a 65533 (tau>=65.533) ne sont pas
c    considerees. 
...
c       read*,buffer 
       ...
       close(unit=13)
c
c   remplacer les valeurs a l'interieur du buffer par des 65533.
c
        do nx=1,nxtcel
        do ny=1,nytcel
...
c
      ...
      open(unit=1,file=filout,status='unknown')
         ...
         if ((dx.eq.0).and.(dy.eq.0)) then
             if ((tau(4,nx,ny).gt.0.).and.(tau(4,nx,ny).lt.65533.)) 
     +       then
             if ((tau(1,nx,ny).gt.0.).and.(tau(1,nx,ny).lt.65533.)) 
     +       then
                 ndat0=ndat0+1
             endif
             endif
     ...
     +   char(48+abs(dy))//'.cxy'
           ...
           open(unit=12,file=name,status='unknown')
              ...
              do iii=1,nxtcel
              do jjj=1,nytcel
          if ((tau(4,iii,jjj).gt.0.).and.(tau(4,iii,jjj).lt.65533.)) 
     +    then
          if ((tau(1,iii,jjj).gt.0.).and.(tau(1,iii,jjj).lt.65533.)) 
     +    then
...
c
           ...
           do nx=1,nxtcel
            do ny=1,nytcel
              ...
              x(i)=tau(1,nx,ny)
              y(i)=tau(4,nx,ny) 
              if ((x(i).gt.0.).and.(x(i).lt.65533.)) then
              if ((y(i).gt.0.).and.(y(i).lt.65533.)) then
                nn=nn+1.
                sumxy=sumxy+x(i)*y(i)
                sumx=sumx+x(i)
                sumy=sumy+y(i)
